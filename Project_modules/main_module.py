...
class main_class:
    ...
    def run_main(self):
        ...
        #model = load_model("Unet_MRI-4.h5",custom_objects={"f1_m":utils.dice_imagewise_m,"precision_m":utils.precision_m,"recall_m":utils.recall_m})
        #model.compile(optimizer="adam", loss="binary_crossentropy", metrics=[utils.dice_imagewise_m, utils.precision_m, utils.recall_m])#Works with 2 classes as output from model.
        
        #Validate model and plot image, mask and prediction
        predictions, mean_f1_score = model_module.test_model(model, valid_gen)   # https://datascience.stackexchange.com/questions/45165/how-to-get-accuracy-f1-precision-and-recall-for-a-keras-model        print('The dice similarity score is:', f1_score)
    ...
    def train_model(self, model, train_generator, validation_generator, epochs):
        ...
        model.compile(optimizer="adam", loss="binary_crossentropy", metrics=[utils.dice_imagewise_m,utils.f1_m], run_eagerly=True)#Works with 2 classes as output from model.        model.summary()
        model.summary()
